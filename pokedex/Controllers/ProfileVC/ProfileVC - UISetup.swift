...
extension ProfileViewController {
    ...
    func setUpLabels() {
        ...
        typesLabel.font = UIFont(name: "Pokemon Classic", size: 14)
        typesLabel.textAlignment = .center
        typesLabel.adjustsFontSizeToFitWidth = true
        view.addSubview(typesLabel)
        
        let statsWidth = view.frame.width/4
        let statsHeight = view.frame.height/30
        
        attackLabel = UILabel(frame: CGRect(x: view.frame.width/10, y: typesLabel.frame.maxY + view.frame.height/60, width: statsWidth, height: statsHeight))
    ...
    func statsLabel(_ label: UILabel) {
        ...
        label.font = UIFont(name: "Pokemon Classic", size: 11)
